...
public:
  ...
  void Mult(const mfem::Vector &x0, const mfem::Vector &y0, mfem::Vector &xf,
            mfem::Vector &yf);
  ...
  void G(const mfem::BlockVector &X, const double theta, mfem::BlockVector &GX,
         int &Geval_err, bool new_pt = true);
  void Residual(const mfem::BlockVector &X, const double theta,
                mfem::BlockVector &r, int &reval_err, bool new_pt = true);
  void ResidualFromG(const mfem::BlockVector &GX, const double theta,
                     mfem::BlockVector &r);
  void PredictorResidual(const mfem::BlockVector &X, const double theta,
                         const double thetaplus, mfem::BlockVector &r,
                         int &reval_err, bool new_pt = true);
  void JacG(const mfem::BlockVector &X, const double theta, bool new_pt = true);
  void NewtonSolve(mfem::BlockOperator &JkOp, const mfem::BlockVector &rk,
                   mfem::BlockVector &dXN);
  void DogLeg(const mfem::BlockOperator &JkOp, const mfem::BlockVector &gk,
              const double delta, const mfem::BlockVector &dXN,
              mfem::BlockVector &dXtr);
  bool FilterCheck(const mfem::Vector &r_comp_norm);
  void UpdateFilter(const mfem::Vector &r_comp_norm);
  void ClearFilter();
  bool NeighborhoodCheck(const mfem::BlockVector &X, const mfem::BlockVector &r,
                         const double theta, const double beta,
                         double &betabar_);
  bool NeighborhoodCheck_1(const mfem::BlockVector &X,
                           const mfem::BlockVector &r, const double theta,
                           const double beta, double &betabar_);
  bool NeighborhoodCheck_2(const mfem::BlockVector &X,
                           const mfem::BlockVector &r, const double theta,
                           const double beta, double &betabar_);
  void SetDeltaMax(const double delta_MAX_) { delta_MAX = delta_MAX_; };
  void SetNeighborhoodParameter(const double beta0_) {
    beta0 = beta0_;
    beta1 = fbeta * beta0;
  };
  void SetContinuationParameter(double theta0_) { theta0 = theta0_; };
  ...
  void SetEarlyTermination(bool earlyTermination_) {
    earlyTermination = earlyTermination_;
  };
  void SetOutputStream(std::ostream *hout_) { hout = hout_; };
