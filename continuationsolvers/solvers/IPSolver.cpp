...
double InteriorPointSolver::MaxStepSize(mfem::Vector &x, mfem::Vector &xl,
                                        mfem::Vector &xhat, double tau) {
  ...
  double alphaMaxglb;
  MPI_Allreduce(&alphaMaxloc, &alphaMaxglb, 1, MPI_DOUBLE, MPI_MIN,
                MPI_COMM_WORLD);
  return alphaMaxglb;
}

double InteriorPointSolver::MaxStepSize(mfem::Vector &x, mfem::Vector &xhat,
                                        double tau) {
...
void InteriorPointSolver::Mult(const mfem::BlockVector &x0,
                               mfem::BlockVector &xf) {
  converged = false;
...
void InteriorPointSolver::lineSearch(mfem::BlockVector &X0,
                                     mfem::BlockVector &Xhat, double mu) {
  ...
  for (int i = 0; i < maxBacktrack; i++) {
    ...
    if (!inFilterRegion) {
      ...
      if (!descentDirection) {
        ...
        switchCondition =
            (alpha * pow(abs(Dxphi0_xhat), sPhi) > delta * pow(thx0, sTheta))
                ? true
                : false;
      }
      if (iAmRoot) {
        ...
        *ipout << "theta(xtrial) = " << thxtrial
               << ", (1-gTheta) *theta(x0) = " << (1. - gTheta) * thx0
               << std::endl;
        *ipout << "phi(xtrial) = " << phxtrial << ", phi(x0) = " << phx0
               << ", phi(x0) - gPhi *theta(x0) = " << phx0 - gPhi * thx0
               << std::endl;
      }
      // Case I
...
void InteriorPointSolver::projectZ(const mfem::Vector &x, mfem::Vector &z,
                                   double mu) {
  double zi;
  double mudivmml;
  for (int i = 0; i < dimM; i++) {
...
double InteriorPointSolver::E(const mfem::BlockVector &x, const mfem::Vector &l,
...
double InteriorPointSolver::E(const mfem::BlockVector &x, const mfem::Vector &l,
                              const mfem::Vector &zl, bool printEeval) {
...
double InteriorPointSolver::phi(const mfem::BlockVector &x, double mu,
  ...
  MPI_Allreduce(&logBarrierLoc, &logBarrierGlb, 1, MPI_DOUBLE, MPI_SUM,
                MPI_COMM_WORLD);
...
void InteriorPointSolver::Dxphi(const mfem::BlockVector &x, double mu,
                                mfem::BlockVector &y) {
...
double InteriorPointSolver::L(const mfem::BlockVector &x, const mfem::Vector &l,
                              const mfem::Vector &zl) {
  double fx = problem->CalcObjective(x);
  mfem::Vector cx(dimC);
  problem->c(x, cx);
  return (fx + InnerProduct(MPI_COMM_WORLD, cx, l) -
          InnerProduct(MPI_COMM_WORLD, x.GetBlock(1), zl));
}

void InteriorPointSolver::DxL(const mfem::BlockVector &x, const mfem::Vector &l,
                              const mfem::Vector &zl, mfem::BlockVector &y) {
...
void InteriorPointSolver::SaveLogBarrierHessianIterates(bool save) {
  MFEM_ASSERT(MyRank == 0 || save == false,
              "currently can only save logbarrier hessian in serial codes");
