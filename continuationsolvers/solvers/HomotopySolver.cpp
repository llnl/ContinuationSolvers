...
HomotopySolver::HomotopySolver(GeneralNLMCProblem *problem_)
    : problem(problem_), block_offsets_xsy(4), block_offsets_xy(3), filter(0),
      krylov_its(0), iterates(0) {
...
void HomotopySolver::Mult(const mfem::Vector &X0, mfem::Vector &Xf) {
  MFEM_VERIFY(X0.Size() == dimx + dimy && Xf.Size() == dimx + dimy,
              "one or more input vectors of incorrect size");
...
void HomotopySolver::Mult(const mfem::Vector &x0, const mfem::Vector &y0,
                          mfem::Vector &xf, mfem::Vector &yf) {
  mfem::BlockVector Xk(block_offsets_xsy);
  Xk = 0.0;
  mfem::BlockVector Xtrial(block_offsets_xsy);
  Xtrial = 0.0;
  mfem::BlockVector dXNk(block_offsets_xsy);
  dXNk = 0.0;
  mfem::BlockVector dXtrk(block_offsets_xsy);
  dXtrk = 0.0;
  mfem::BlockVector GX(block_offsets_xsy);
  GX = 0.0;
  mfem::BlockVector GX0(block_offsets_xsy);
  GX0 = 0.0;
  mfem::BlockVector rk(block_offsets_xsy);
  rk = 0.0;
  mfem::BlockVector rktrial(block_offsets_xsy);
  rktrial = 0.0;
  mfem::BlockVector rklinear(block_offsets_xsy);
  rklinear = 0.0;
  mfem::BlockVector gk(block_offsets_xsy);
  ...
  MFEM_VERIFY(Feval_err == 0 && Qeval_err == 0,
              "unsuccessful evaluation of F and/or Q at initial point of "
              "Homotopy solver");
  ...
  while (jOpt < max_outer_iter) {
    ...
    MFEM_VERIFY(
        Eeval_err == 0,
        "error in evaluation of optimality error E, should not occur\n");
    ...
    NewtonSolve(*JGX, rk,
                dXNk); // Newton direction, associated to equation rk = 0

    JGX->MultTranspose(rk, gk);
    gk *= 2.0; // gradient of quadratic-model (\nabla_{dX}(||rk + Jk *
               // dX||_2)^2)_{|dX=0}= 2 Jk^T rk
    ...
    mfem::Vector rktrial_comp_norm(3);
    rktrial_comp_norm = 0.0;
    for (int i = 0; i < 3; i++) {
      rktrial_comp_norm(i) =
          mfem::GlobalLpNorm(2, rktrial.GetBlock(i).Norml2(), MPI_COMM_WORLD);
    }
    ...
    for (int itr_centering = 0; itr_centering < max_tr_centering;
         itr_centering++) {
      MFEM_VERIFY(delta > 1.e-30,
                  "loss of accuracy in dog-leg (TR radius too small)");
      if (!tr_centering) {
        break;
      }
      ...
      if (reval_err > 0) {
        if (iAmRoot && print_level > 0) {
          *hout << "TRcen -- bad evaluation of residual (reducing trust-region "
                   "radius)\n";
        }
        delta *= 0.5;
        continue;
      ...
      double rktrial_sqrnorm =
          mfem::InnerProduct(MPI_COMM_WORLD, rktrial, rktrial);
      double rklinear_sqrnorm =
          mfem::InnerProduct(MPI_COMM_WORLD, rklinear, rklinear);
      double pred_decrease =
          rk_sqrnorm - rklinear_sqrnorm; // || rk ||_2^2 - || rk + Jk * dx||_2^2
      double actual_decrease =
          rk_sqrnorm -
          rktrial_sqrnorm; // || r(Xk) ||_2^2 - || r(Xk + dX) ||_2^2
      ...
      if (rhok < eta1) {
        for (int i = 0; i < 3; i++) {
          rktrial_comp_norm(i) = mfem::GlobalLpNorm(
              2, rktrial.GetBlock(i).Norml2(), MPI_COMM_WORLD);
        }
        inFilterRegion = FilterCheck(rktrial_comp_norm);
        inNeighborhood =
            NeighborhoodCheck(Xtrial, rktrial, theta, beta1, betabar);
        if (iAmRoot && print_level > 0) {
          if (inFilterRegion) {
        ...
        if (iAmRoot && print_level > 0) {
          *hout << "TRcen -- accepted trial point, potentially increasing "
                   "TR-radius\n";
        }
    ...
    MFEM_VERIFY(reval_err == 0,
                "bad residual evaluation, this should have been caught\n");
    ...
    if (inNeighborhood) {
      ...
      mfem::BlockVector rkp(block_offsets_xsy);
      rkp = 0.0;
      mfem::BlockVector dXp(block_offsets_xsy);
      dXp = 0.0;
      mfem::BlockVector Xtrialp(block_offsets_xsy);
      Xtrialp = 0.0;
      ...
      while (!(linesearch_converged) && i_linesearch < max_linesearch_steps) {
        ...
        inNeighborhood =
            NeighborhoodCheck(Xtrialp, rktrial, theta_t, beta1, betabar);
        if (inNeighborhood) {
      ...
      if (iAmRoot && print_level > 0) {
        ...
        *hout << "CenManagement -- applying heuristics for quick termination "
                 "resolution\n";
      }
      beta0 = fbeta * betabar;
      beta1 = fbeta * beta0;
      ...
      if (gk_norm < theta * epsgrad) {
        if (iAmRoot && print_level > 0) {
          if (earlyTermination) {
            *hout << "Exiting -- converged to a local stationary point of "
                     "||rk||_2^2\n";
          } else {
            *hout << "Warning -- apparent convergence to a local stationary "
                     "point of ||rk||_2^2\n";
          }
        }
        if (earlyTermination) {
        ...
        double Xk_norm =
            mfem::GlobalLpNorm(mfem::infinity(), Xk.Normlinf(), MPI_COMM_WORLD);
        if (Xk_norm > deltabnd) {
          if (iAmRoot && print_level > 0) {
            if (earlyTermination) {
...
void HomotopySolver::G(const mfem::BlockVector &X, const double theta,
                       mfem::BlockVector &GX, int &Geval_err, bool new_pt) {
  int Feval_err = 0;
  int Qeval_err = 0;
  mfem::Vector tempx(dimx);
  tempx = 0.0;
  // compute sqrt((x-s)^2 + 4(theta a)^q) term
  for (int i = 0; i < dimx; i++) {
    tempx(i) = std::sqrt(std::pow(X(i) - X(i + dimx), 2) +
                         4.0 * std::pow(theta * ax(i), q));
  }
...
double HomotopySolver::E(const mfem::BlockVector &X, int &Eeval_err,
  ...
  mfem::Vector xsc(dimx);
  xsc = 0.0;
  mfem::Vector ssc(dimx);
  ssc = 0.0;
  for (int i = 0; i < dimx; i++) {
  ...
  double xsc_infnorm =
      mfem::GlobalLpNorm(mfem::infinity(), xsc.Normlinf(), MPI_COMM_WORLD);
  double ssc_infnorm =
      mfem::GlobalLpNorm(mfem::infinity(), ssc.Normlinf(), MPI_COMM_WORLD);
  ...
  mfem::BlockVector r0(block_offsets_xsy);
  r0 = 0.0;
  Residual(X, 0.0, r0, Eeval_err, new_pt); // residual at \theta = 0

  mfem::Array<double> r0_infnorms(3);
  for (int i = 0; i < 3; i++) {
    r0_infnorms[i] = mfem::GlobalLpNorm(
        mfem::infinity(), r0.GetBlock(i).Normlinf(), MPI_COMM_WORLD);
  }
  mfem::Vector xs(dimx);
  xs = 0.0;
  xs.Set(1.0, x);
  xs *= s;
  double xs_infnorm =
      mfem::GlobalLpNorm(mfem::infinity(), xs.Normlinf(), MPI_COMM_WORLD);

  double Err = 0.;
  if (dimxglb > 0) {
    Err = std::max(std::min(r0_infnorms[0] / fz, xs_infnorm / Msc),
                   std::max(r0_infnorms[1], r0_infnorms[2]) / fz);
  } else {
    Err = r0_infnorms[2];
  }
  return Err;
...
void HomotopySolver::Residual(const mfem::BlockVector &X, const double theta,
                              mfem::BlockVector &r, int &reval_err,
                              bool new_pt) {
...
void HomotopySolver::ResidualFromG(const mfem::BlockVector &GX,
                                   const double theta, mfem::BlockVector &r) {
  r.Set(1.0, GX);
  r.GetBlock(0).Add(-theta, bx);
  r.GetBlock(1).Add(-theta, cx);
  r.GetBlock(2).Add(-theta, cy);
...
void HomotopySolver::PredictorResidual(const mfem::BlockVector &X,
                                       const double theta,
                                       const double thetaplus,
                                       mfem::BlockVector &r, int &reval_err,
                                       bool new_pt) {
  G(X, theta, r, reval_err, new_pt);
 ...
 *                       [dG_(3,1)     0        dG_(3,3)]
 */
void HomotopySolver::JacG(const mfem::BlockVector &X, const double theta,
                          bool new_pt) {
  // I - diag( (x - s) / sqrt( (x - s)^2 + 4 * (\theta a)^q)
  mfem::Vector diagJacGxx(dimx);
  diagJacGxx = 0.0;
  for (int i = 0; i < dimx; i++) {
    diagJacGxx(i) =
        1.0 - (X(i) - X(i + dimx)) / std::sqrt(std::pow(X(i) - X(i + dimx), 2) +
                                               4. * std::pow(theta * ax(i), q));
  }
  if (JGxx) {
    delete JGxx;
    JGxx = nullptr;
  ...
  dFdx = dynamic_cast<mfem::HypreParMatrix *>(
      problem->DxF(X.GetBlock(0), X.GetBlock(2)));
  mfem::Vector diagtgx(dimx);
  diagtgx = 0.0;
  for (int i = 0; i < dimx; i++) {
    diagtgx(i) = pow(theta * gammax(i), p);
  }
  mfem::HypreParMatrix *Dtgx =
      GenerateHypreParMatrixFromDiagonal(dofOffsetsx, diagtgx);
  JGsx = mfem::Add(-1.0, *dFdx, -1.0, *Dtgx);
  delete Dtgx;

  // d / ds (G_t)_2 = I
  mfem::Vector one(dimx);
  one = 1.0;
  if (JGss) {
    delete JGss;
  }
  ...
  temp_mat = dynamic_cast<mfem::HypreParMatrix *>(
      problem->DyF(X.GetBlock(0), X.GetBlock(2)));
  ...
  temp_mat = dynamic_cast<mfem::HypreParMatrix *>(
      problem->DxQ(X.GetBlock(0), X.GetBlock(2)));
  ...
  dQdy = dynamic_cast<mfem::HypreParMatrix *>(
      problem->DyQ(X.GetBlock(0), X.GetBlock(2)));
  mfem::Vector diagtgy(dimy);
  diagtgy = 0.0;
  for (int i = 0; i < dimy; i++) {
    diagtgy(i) = std::pow(theta * gammay(i), p);
  }
  mfem::HypreParMatrix *Dtgy =
      GenerateHypreParMatrixFromDiagonal(dofOffsetsy, diagtgy);
  ...
  JGX->SetBlock(0, 0, JGxx);
  JGX->SetBlock(0, 1, JGxs);
  JGX->SetBlock(1, 0, JGsx);
  JGX->SetBlock(1, 1, JGss);
  JGX->SetBlock(1, 2, JGsy);
  JGX->SetBlock(2, 0, JGyx);
  JGX->SetBlock(2, 2, JGyy);
}

// solve J dX_N = - rk
void HomotopySolver::NewtonSolve(mfem::BlockOperator &JkOp,
                                 const mfem::BlockVector &rk,
                                 mfem::BlockVector &dXN) {
  if (linSolver) {
    linSolver->SetOperator(JkOp);
    linSolver->Mult(rk, dXN);
    dXN *= -1.0;
    mfem::IterativeSolver *itSolver =
        dynamic_cast<mfem::IterativeSolver *>(linSolver);
    if (!itSolver) {
      CondensedHomotopySolver *condensedhomotopysolver =
          dynamic_cast<CondensedHomotopySolver *>(linSolver);
      if (condensedhomotopysolver) {
        itSolver = dynamic_cast<mfem::IterativeSolver *>(
            condensedhomotopysolver->GetReducedSolver());
      }
    }
    ...
    mfem::Array2D<const mfem::HypreParMatrix *> JkBlockMat(num_row_blocks,
                                                           num_col_blocks);
    for (int i = 0; i < num_row_blocks; i++) {
      for (int j = 0; j < num_col_blocks; j++) {
        if (!JkOp.IsZeroBlock(i, j)) {
          JkBlockMat(i, j) =
              dynamic_cast<mfem::HypreParMatrix *>(&(JkOp.GetBlock(i, j)));
...
void HomotopySolver::DogLeg(const mfem::BlockOperator &JkOp,
                            const mfem::BlockVector &gk, const double delta,
                            const mfem::BlockVector &dXN,
                            mfem::BlockVector &dXtr) {
  ...
  if (dXN_norm <= delta) {
    ...
    mfem::BlockVector dXsd(block_offsets_xsy);
    dXsd = 0.0;
    mfem::BlockVector Jkgk(block_offsets_xsy);
    Jkgk = 0.0;
    JkOp.Mult(gk, Jkgk);
    double gk_norm = mfem::GlobalLpNorm(2, gk.Norml2(), MPI_COMM_WORLD);
    ...
    if (dXsd_norm >= delta) {
      ...
      MFEM_VERIFY(discr >= 0. && a >= 0.,
                  "loss of accuracy: Gauss-Newton model not convex?!?");
      if (b > 0.) {
...
bool HomotopySolver::FilterCheck(const mfem::Vector &r_comp_norm) {
  ...
  for (int i = 0; i < filter.Size(); i++) {
    MFEM_VERIFY(filter[i]->Size() == 3,
                "each element of filter must be a 3-vector");
    for (int j = 0; j < 3; j++) {
  ...
  // reduction is achieved ||r_theta_i|| < alpha_i - gamma_f ||(alpha1, alpha2,
  // alpha3)|| if reduction is achieved then we are not in the filtered region
...
bool HomotopySolver::NeighborhoodCheck(const mfem::BlockVector &X,
...
bool HomotopySolver::NeighborhoodCheck_1(const mfem::BlockVector & /*X*/,
                                         const mfem::BlockVector &r,
                                         const double theta, const double beta,
                                         double &betabar_) {
  double r_inf_norm =
      mfem::GlobalLpNorm(mfem::infinity(), r.Normlinf(), MPI_COMM_WORLD);
...
bool HomotopySolver::NeighborhoodCheck_2(const mfem::BlockVector &X,
                                         const mfem::BlockVector &r,
                                         const double theta, const double beta,
                                         double &betabar_) {
  double x_inf_norm = mfem::GlobalLpNorm(
      mfem::infinity(), X.GetBlock(0).Normlinf(), MPI_COMM_WORLD);
  double s_inf_norm = mfem::GlobalLpNorm(
      mfem::infinity(), X.GetBlock(1).Normlinf(), MPI_COMM_WORLD);
  double xs_inf_norm = std::max(x_inf_norm, s_inf_norm);

  mfem::Vector r_inf_norms(3);
  r_inf_norms = 0.0;
  for (int i = 0; i < 3; i++) {
    r_inf_norms(i) = mfem::GlobalLpNorm(
        mfem::infinity(), r.GetBlock(i).Normlinf(), MPI_COMM_WORLD);
  }

  bool inNeighborhood =
      ((r_inf_norms(0) <= beta * theta * xs_inf_norm) &&
       (std::max(r_inf_norms(1), r_inf_norms(2)) <= beta * theta));
  betabar_ = std::max(r_inf_norms(0) / (theta * xs_inf_norm),
                      std::max(r_inf_norms(1), r_inf_norms(2)) / theta);
  return inNeighborhood;
}
