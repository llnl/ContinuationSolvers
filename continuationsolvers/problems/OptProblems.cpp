...
void GeneralOptProblem::getLumpedMassWeights(mfem::Vector& Mu, mfem::Vector& Mm, mfem::Vector& Mc)
{
  MFEM_VERIFY(Mu.Size() == Mulump.Size(), "size issue");
  MFEM_VERIFY(Mm.Size() == Mmlump.Size(), "size issue");
  MFEM_VERIFY(Mc.Size() == Mclump.Size(), "size issue");
  Mu.Set(1.0, Mulump);
  Mm.Set(1.0, Mmlump);
  Mc.Set(1.0, Mclump);
}
...
double OptProblem::CalcObjective(const mfem::BlockVector& x, int& eval_err) { return E(x.GetBlock(0), eval_err); }
...
mfem::Operator* OptProblem::Duuf(const mfem::BlockVector& x) { return DddE(x.GetBlock(0)); }
...
mfem::Operator* OptProblem::Duc(const mfem::BlockVector& x) { return Ddg(x.GetBlock(0)); }

mfem::Operator* OptProblem::Dmc(const mfem::BlockVector& /*x*/) { return Ih; }

mfem::Operator* OptProblem::Duucl(const mfem::BlockVector& x, const mfem::Vector& l) { return Dddgl(x.GetBlock(0), l); }
...
mfem::Operator* OptProblem::Dmucl(const mfem::BlockVector& /*x*/, const mfem::Vector& /*l*/) { return nullptr; }

mfem::Operator* OptProblem::Dmmcl(const mfem::BlockVector& /*x*/, const mfem::Vector& /*l*/) { return nullptr; }
...
ReducedOptProblem::ReducedOptProblem(OptProblem* problem_, HYPRE_Int* constraintMask)
{
  ...
  auto Rc_mat = GenerateProjector(constraintOffsets_reduced, constraintOffsets, constraintMask);
  Rc.reset(Rc_mat);
  ...
  dDC.SetSize(nDofs);
  dDC = 0.1;
  gradEfull.SetSize(nDofs);
  gradEfull = 0.0;
  dFull.SetSize(nDofs);
  dFull = 0.0;
  gfull.SetSize(problem->GetDimM());
  gfull = 0.0;
}

ReducedOptProblem::ReducedOptProblem(OptProblem* problem_, HYPRE_Int* dofMask, HYPRE_Int* constraintMask)
{
  problem = problem_;
  ...
  delete[] dofOffsets;
  dDC.SetSize(nProblemDofs);
  dDC = 0.0;
  gradEfull.SetSize(nProblemDofs);
  gradEfull = 0.0;
  dFull.SetSize(nProblemDofs);
  dFull = 0.0;
  gfull.SetSize(nProblemConstraints);
  gfull = 0.0;
}
...
void ReducedOptProblem::ProlongateToFullDofs(const mfem::Vector& d, mfem::Vector& df) { Pdof->Mult(d, df); }
...
double OptEqProblem::CalcObjective(const mfem::BlockVector& x, int& eval_err) { return E(x.GetBlock(0), eval_err); }
...
mfem::Operator* OptEqProblem::Duuf(const mfem::BlockVector& x) { return DddE(x.GetBlock(0)); }
...
// c(u, m) = g(u)
void OptEqProblem::c(const mfem::BlockVector& x, mfem::Vector& y, int& eval_err) { g(x.GetBlock(0), y, eval_err); }

mfem::Operator* OptEqProblem::Duc(const mfem::BlockVector& x) { return Ddg(x.GetBlock(0)); }

mfem::Operator* OptEqProblem::Dmc(const mfem::BlockVector& /*x*/) { return dcdm.get(); }
...
mfem::Operator* OptEqProblem::Dumcl(const mfem::BlockVector& /*x*/, const mfem::Vector& /*l*/) { return nullptr; }

mfem::Operator* OptEqProblem::Dmucl(const mfem::BlockVector& /*x*/, const mfem::Vector& /*l*/) { return nullptr; }

mfem::Operator* OptEqProblem::Dmmcl(const mfem::BlockVector& /*x*/, const mfem::Vector& /*l*/) { return nullptr; }
...
mfem::Operator* ParamOptProblem::DddE(const mfem::Vector& d) { return DddE(d, theta_default); }

mfem::Operator* ParamOptProblem::DdddEl(const mfem::Vector& /*d*/, const mfem::Vector& /*l*/,
...
mfem::Operator* ParamOptProblem::DthddEl(const mfem::Vector& /*d*/, const mfem::Vector& /*l*/,
                                         const mfem::Vector& /*theta*/)
{
  if (!HthddEl.get()) {
...
mfem::Operator* ParamOptProblem::DththdEl(const mfem::Vector& /*d*/, const mfem::Vector& /*l*/,
                                          const mfem::Vector& /*theta*/)
{
  if (!HththdEl.get()) {
...
void ParamOptProblem::g(const mfem::Vector& d, mfem::Vector& gd, int& eval_err) { g(d, theta_default, gd, eval_err); }

mfem::Operator* ParamOptProblem::Ddg(const mfem::Vector& d) { return Ddg(d, theta_default); }
...
mfem::Operator* ParamOptProblem::Ddthgl(const mfem::Vector& /*d*/, const mfem::Vector& /*l*/,
                                        const mfem::Vector& /*theta*/)
{
  if (!Hdthgl.get()) {
...
mfem::Operator* ParamOptProblem::Dththgl(const mfem::Vector& /*d*/, const mfem::Vector& /*l*/,
                                         const mfem::Vector& /*theta*/)
{
  if (!Hththgl.get()) {
...
mfem::Operator* ParamOptProblem::Dddgl2(const mfem::Vector& /*d*/, const mfem::Vector& /*l*/,
                                        const mfem::Vector& /*theta*/)
{
  if (!Hddgl2.get()) {
...
mfem::Operator* ParamOptProblem::Dthdgl2(const mfem::Vector& /*d*/, const mfem::Vector& /*l*/,
                                         const mfem::Vector& /*theta*/)
{
  if (!Hthdgl2.get()) {
...
ReducedParamOptProblem::ReducedParamOptProblem(ParamOptProblem* problem_, mfem::Array<int> tdof_list)
{
  ...
  mfem::Vector theta_init(dimU);
  theta_init = 0.0;
  Rc->Mult(problem->GetTheta(), theta_init);
  InitTheta(theta_init);

  dDC.SetSize(problem->GetDimU());
  dDC = 0.0;
  dFull.SetSize(problem->GetDimU());
  dFull = 0.0;
  gradEFull.SetSize(problem->GetDimU());
  gradEFull = 0.0;
  gFull.SetSize(problem->GetDimM());
  gFull = 0.0;
  thetaFull.SetSize(problem->GetDimM());
  thetaFull = 0.0;
}

double ReducedParamOptProblem::E(const mfem::Vector& d, const mfem::Vector& theta, int& eval_err)
{
  Pdof->Mult(d, dFull);
  dFull.Add(1.0, dDC);
...
void ReducedParamOptProblem::DdE(const mfem::Vector& d, const mfem::Vector& theta, mfem::Vector& gradE)
{
  Pdof->Mult(d, dFull);
  dFull.Add(1.0, dDC);
  Pc->Mult(theta, thetaFull);
...
void ReducedParamOptProblem::g(const mfem::Vector& d, const mfem::Vector& theta, mfem::Vector& gd, int& eval_err)
{
  Pdof->Mult(d, dFull);
  dFull.Add(1.0, dDC);
  Pc->Mult(theta, thetaFull);
...
void ReducedParamOptProblem::ProlongateToFullDofs(const mfem::Vector& d, mfem::Vector& df) { Pdof->Mult(d, df); }
