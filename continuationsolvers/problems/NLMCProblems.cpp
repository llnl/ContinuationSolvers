...
void GeneralNLMCProblem::Init(HYPRE_BigInt *dofOffsetsx_,
                              HYPRE_BigInt *dofOffsetsy_) {
...
// F(x, y) = g(y)
void OptNLMCProblem::F(const mfem::Vector &x, const mfem::Vector &y,
                       mfem::Vector &feval, int &eval_err,
                       bool /*new_pt*/) const {
...
void OptNLMCProblem::Q(const mfem::Vector &x, const mfem::Vector &y,
                       mfem::Vector &qeval, int &eval_err,
                       bool /*new_pt*/) const {
...
// dF/dx = 0
mfem::Operator *OptNLMCProblem::DxF(const mfem::Vector & /*x*/,
                                    const mfem::Vector & /*y*/,
                                    bool /*new_pt*/) {
  return dFdx;
}
...
// dF/dy = dg/dy
mfem::Operator *OptNLMCProblem::DyF(const mfem::Vector & /*x*/,
                                    const mfem::Vector &y, bool /*new_pt*/) {
  return optproblem->Ddg(y);
}

// dQ/dx = -(dg/dy)^T
mfem::Operator *OptNLMCProblem::DxQ(const mfem::Vector & /*x*/,
                                    const mfem::Vector &y, bool /*new_pt*/) {
  mfem::Operator *J = optproblem->Ddg(y);
...
mfem::Operator *OptNLMCProblem::DyQ(const mfem::Vector & /*x*/,
                                    const mfem::Vector &y, bool /*new_pt*/) {
...
EqualityConstrainedHomotopyProblem::EqualityConstrainedHomotopyProblem(
    mfem::Array<int> fixed_tdof_list, mfem::Array<int> disp_tdof_list,
    const mfem::Vector uDC) {
  EqualityConstrainedHomotopyInit();
...
mfem::Vector
EqualityConstrainedHomotopyProblem::GetLagrangeMultiplier(mfem::Vector &X) {
  MFEM_VERIFY(X.Size() == dimx + dimy, "input vector of an invalid size");
...
void EqualityConstrainedHomotopyProblem::SetSizes(int dimu, int dimc) {
  ...
  std::unique_ptr<HYPRE_BigInt[]> cOffsets =
      std::make_unique<HYPRE_BigInt[]>(2);
...
void EqualityConstrainedHomotopyProblem::SetSizes(HYPRE_BigInt *uOffsets,
  ...
  // dF / dx 0 x 0 matrix
  {
    ...
    dFdx =
        GenerateHypreParMatrixFromSparseMatrix(dofOffsetsx, dofOffsetsx, temp);
    delete temp;
  }
  ...
  // dF / dy 0 x dimy matrix
  {
    ...
    dFdy =
        GenerateHypreParMatrixFromSparseMatrix(dofOffsetsx, dofOffsetsy, temp);
    delete temp;
  }
  ...
  // dQ / dx dimy x 0 matrix
  {
    ...
    dQdx =
        GenerateHypreParMatrixFromSparseMatrix(dofOffsetsy, dofOffsetsx, temp);
    delete temp;
  }
  q_cache.SetSize(dimy);
  q_cache = 0.0;
  ...
  std::unique_ptr<HYPRE_Int[]> mask =
      std::make_unique<HYPRE_Int[]>(static_cast<size_t>(dimufull_));
  ...
  std::unique_ptr<HYPRE_Int[]> dispmask =
      std::make_unique<HYPRE_Int[]>(static_cast<size_t>(dimufull_));
  for (int i = 0; i < dimufull_; i++) {
  ...
  dispuOffsets.reset(
      offsetsFromLocalSizes(disp_tdof_list_.Size(), MPI_COMM_WORLD));
...
void EqualityConstrainedHomotopyProblem::F(const mfem::Vector &x,
                                           const mfem::Vector &y,
                                           mfem::Vector &feval, int &Feval_err,
                                           bool /*new_pt*/) const {
...
// Q = [  r + (dc/du)^T l]
//     [ -c ]
void EqualityConstrainedHomotopyProblem::Q(const mfem::Vector &x,
                                           const mfem::Vector &y,
                                           mfem::Vector &qeval, int &Qeval_err,
                                           bool new_pt) const {
  MFEM_VERIFY(set_sizes, "need to set sizes in problem constructor");
  MFEM_VERIFY(x.Size() == dimx && y.Size() == dimy && qeval.Size() == dimy,
  ...
  if (new_pt) {
    ...
    try {
      ...
      // if the point is not new then it can choose to not compute a new
      // derivative
      bool new_constraint_pt = false;
      bool new_constraint_deriv = true;
      auto residual_contribution = constraintJacobianTvp(
          ufull_, l, new_constraint_pt, new_constraint_deriv);
      if (has_essential_dofs) {
  ...
  if (Qeval_err == 0) {
    ...
    MPI_Allreduce(&Qeval_err_loc, &Qeval_err, 1, MPI_INT, MPI_MAX,
                  MPI_COMM_WORLD);
  }
};
...
mfem::Operator *
EqualityConstrainedHomotopyProblem::DyQ(const mfem::Vector & /*x*/,
                                        const mfem::Vector &y, bool new_pt) {
  MFEM_VERIFY(set_sizes, "need to set sizes in problem constructor");
...
void EqualityConstrainedHomotopyProblem::SetAdjointSolver(
    mfem::Solver *adjoint_solver_) {
...
void EqualityConstrainedHomotopyProblem::AdjointSolve(
    const mfem::Vector &evaluation_u_point, const mfem::Vector &adjoint_load,
    mfem::Vector &adjoint) {
  MFEM_VERIFY(adjoint_load.Size() == dimu_ + dimc_,
              "Adjoint load not of the correct size");
  MFEM_VERIFY(adjoint.Size() == dimu_ + dimc_,
              "Adjoint solution vector not of the correct size");
  mfem::BlockVector evaluation_y_point(y_partition);
  evaluation_y_point = 0.0;
  evaluation_y_point.GetBlock(0).Set(1.0, evaluation_u_point);
  mfem::Vector evaluation_x_point(dimx);
  evaluation_x_point = 0.0;
...
// void EqualityConstrainedHomotopyProblem::fullDisplacement(const mfem::Vector&
// X, mfem::Vector& u)
//{
...
void InequalityConstrainedHomotopyProblem::SetSizes(HYPRE_BigInt *uOffsets,
                                                    HYPRE_BigInt *cOffsets) {
  set_sizes = true;
  dimu_ = uOffsets[1] - uOffsets[0];
  dimc_ = cOffsets[1] - cOffsets[0];
...
void InequalityConstrainedHomotopyProblem::F(const mfem::Vector &x,
                                             const mfem::Vector &y,
                                             mfem::Vector &feval,
                                             int &Feval_err,
                                             bool new_pt) const {
...
// Q(l, u) = (r(u) - (dc/du)^T l)
void InequalityConstrainedHomotopyProblem::Q(const mfem::Vector &x,
                                             const mfem::Vector &y,
                                             mfem::Vector &qeval,
                                             int &Qeval_err,
                                             bool new_pt) const {
  ...
  if (new_pt) {
    ...
    try {
      ...
      auto residual_contribution = constraintJacobianTvp(
          ufull_, x, new_constraint_pt, new_constraint_deriv);
...
mfem::Operator *
InequalityConstrainedHomotopyProblem::DyF(const mfem::Vector & /*x*/,
                                          const mfem::Vector &y, bool new_pt) {
  MFEM_VERIFY(set_sizes, "need to set sizes in problem constructor");
  MFEM_VERIFY(y.Size() == dimy,
              "InertialReliefProblem::DyF -- Inconsistent dimensions");
...
// recomputation may be avoided in the residualJacobian/constraintJacobian calls
mfem::Operator *
InequalityConstrainedHomotopyProblem::DxQ(const mfem::Vector & /*x*/,
                                          const mfem::Vector &y, bool new_pt) {
  MFEM_VERIFY(set_sizes, "need to set sizes in problem constructor");
  MFEM_VERIFY(y.Size() == dimy,
              "InertialReliefProblem::DyQ -- Inconsistent dimensions");
...
// recomputation may be avoided in the residualJacobian/constraintJacobian calls
mfem::Operator *
InequalityConstrainedHomotopyProblem::DyQ(const mfem::Vector & /*x*/,
                                          const mfem::Vector &y, bool new_pt) {
  MFEM_VERIFY(set_sizes, "need to set sizes in problem constructor");
  MFEM_VERIFY(y.Size() == dimy,
              "InertialReliefProblem::DyQ -- Inconsistent dimensions");
