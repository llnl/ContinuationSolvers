...
class MPECProblem : public OptEqProblem {
 protected:
  ...
  mfem::Array<int> primal_blockoffsets;      // offsets for variable U = (u, p, \theta)
                                             // or U = (u, p, \theta, s, z) depending on how use_slacks is set
  mfem::Array<int> constraint_blockoffsets;  // offsets for constraints which
                                             // have sizes like (u, p)
                                             // or (u, p, s, z) depending on how use_slacks is set
 ...
 public:
  MPECProblem(ParamOptProblem* paramopt_, bool use_slack = false);
  void RegularizedComplementarity(const mfem::Vector& s, const mfem::Vector& z, const double& mu, mfem::Vector& phi);
  mfem::Operator* DsRegularizedComplementarity(const mfem::Vector& s, const mfem::Vector& z, const double& mu);
  mfem::Operator* DzRegularizedComplementarity(const mfem::Vector& s, const mfem::Vector& z, const double& mu);
  mfem::Operator* DsslRegularizedComplementarity(const mfem::Vector& s, const mfem::Vector& z, const mfem::Vector& l,
                                                 const double& mu);
  mfem::Operator* DszlRegularizedComplementarity(const mfem::Vector& s, const mfem::Vector& z, const mfem::Vector& l,
                                                 const double& mu);
  mfem::Operator* DzzlRegularizedComplementarity(const mfem::Vector& s, const mfem::Vector& z, const mfem::Vector& l,
                                                 const double& mu);
...
class ObstacleDesignProblem : public MPECProblem {
 public:
  ObstacleDesignProblem(ParamOptProblem* paramopt_, bool use_slacks = false);
  double E(const mfem::Vector& U, int& eval_err) override;
