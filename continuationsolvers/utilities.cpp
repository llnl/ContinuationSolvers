...
GenerateHypreParMatrixFromSparseMatrix(HYPRE_BigInt *rowOffsetsloc,
  ...
  mfem::HypreParMatrix *Ahypre =
      new mfem::HypreParMatrix(MPI_COMM_WORLD, nrows_loc, nrows_glb, ncols_glb,
                               AI, AJ, Adata, rowOffsetsloc, colOffsetsloc);
  return Ahypre;
}
...
GenerateHypreParMatrixFromDiagonal(HYPRE_BigInt *offsetsloc,
  ...
  Dhypre =
      GenerateHypreParMatrixFromSparseMatrix(offsetsloc, offsetsloc, Dsparse);
  delete Dsparse;
  return Dhypre;
}
...
mfem::HypreParMatrix *GenerateProjector(HYPRE_BigInt *reduced_offsets,
  ...
  Phypre =
      GenerateHypreParMatrixFromSparseMatrix(reduced_offsets, offsets, Psparse);
  delete Psparse;
  return Phypre;
}
...
HYPRE_BigInt *offsetsFromLocalSizes(int n, MPI_Comm comm) {
  ...
  for (int i = 0; i < myrank; i++) {
    tag = myrank + i * nprocs;
    MPI_Recv(&(helper[i]), 1, MPI_INT, i, tag, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
    offsets[0] += helper[i];
  }
...
mfem::HypreParMatrix *NonZeroRowMap(const mfem::HypreParMatrix &A) {
  ...
  return new mfem::HypreParMatrix(comm, numNZRows, glob_nrows, glob_ncols,
                                  nzRowMap.GetI(), nzRowMap.GetJ(),
                                  nzRowMap.GetData(), rows_part, cols_part);
...
DirectSolver::DirectSolver() : mfem::Solver(), solver(nullptr) {}

DirectSolver::DirectSolver(const mfem::Operator &op) : DirectSolver() {
  SetOperator(op);
}
...
void DirectSolver::SetOperator(const mfem::Operator &op) {
  ...
  directSolver->SetOperator(*op_ptr);
#else
  MFEM_ABORT("DirectSolver will not work unless compiled mfem is with MUMPS, "
             "MKL_CPARDISO, or STRUMPACK");
#endif
#if defined(MFEM_USE_STRUMPACK) || defined(MFEM_USE_MUMPS) ||                  \
...
mfem::HypreParMatrix *GenerateNullHypreParMatrix(HYPRE_BigInt *rowOffsets,
  ...
  auto temp =
      std::unique_ptr<mfem::SparseMatrix>(new mfem::SparseMatrix(m, N, nnz));

  // generate HypreParMatrix from SparseMatrix
  A = GenerateHypreParMatrixFromSparseMatrix(rowOffsets, colOffsets,
                                             temp.get());
  return A;
}
